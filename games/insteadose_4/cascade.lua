...
lamp = obj{
	...
	silence_act = function(s)
		if s.click == 0 then
			p "Неправильная тишина. Кроме слегка учащенного дыхание -- никаких звуков. Так не бывает...";			
		elseif s.click == 1 then
			p "..."
	...
	act = function(s)
		if not trigger._checked then
			...
			switch(s.examine){
				...
				def = "Не будет считаться с моими желаниями до тех пор, пока не подтвержу свое право на такове кодом с ключ-карты.";
			}
			if s.examine < 3 then
...
anastasia_dlg = dlg{
	...
	phr = {
		...
		{tag = 'why', false, "Гермоврата, они перегоражавиют...", txtem"Вдруг по коридору проносится гулкое \"БАХ\"", [[poff 'what'; pon 'bang']]};
...
game_intro = [[Нужна привычка чтобы спокойно спать в тишине. Ведь есть что-то гнетущее в безмолвии мира, что заставляет веки распахнуться, а взгляд -- тщетно метаться в вязкой ночи. И по какой-то причине воспринимаемая в тот момент пустота не дарует закономерного облегчения -- а лишь настораживает еще больше.^
	Лишь потом, после окончательного пробуждения, приходит сбивающие с толку осознание, что в спальне никого нет и быть не может. Мало того, что это бессмысленно, -- так она еще и отгорожена запертой дверью!^^
